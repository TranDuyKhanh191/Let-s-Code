...
import { api } from "../../services/api"; // Đảm bảo import api từ đúng đường dẫn
...
// ============================================================================
// 2. UI COMPONENTS (Tái sử dụng style chuẩn)
// ============================================================================
...
const ErrorToast = ({ message, onClose }: { message: string; onClose: () => void }) => (
  <div className="flex items-start w-full gap-3 p-4 mb-6 text-sm border border-red-100 shadow-sm bg-red-50 rounded-xl animate-fade-in-down">
    ...
    <div className="flex-1">
      <h4 className="font-bold text-red-800">Yêu cầu thất bại</h4>
      <p className="mt-1 leading-relaxed text-red-600">{message}</p>
    </div>
    <button onClick={onClose} className="text-red-400 transition-colors hover:text-red-600">
  ...
  return (
    <div className="relative group">
      ...
      <input
        ...
        className={`peer block w-full pl-12 pr-4 pt-6 pb-2 rounded-xl border-2 bg-gray-50/50 outline-none text-gray-900 transition-all duration-300
          ${isFocused 
...
export default function ForgotPasswordPage() {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // --- GỌI API THẬT ĐỂ GỬI OTP ---
      // Endpoint này sẽ kích hoạt Backend gửi email OTP
      await api.post("/auth/forgot-password", { email });
      
      // Nếu thành công (không throw error), chuyển hướng sang trang Verify OTP
      // và mang theo Email để trang sau tự động điền
      nav("/verify-otp", { state: { email: email } });

    } catch (err: any) {
      console.error(err);
      // Hiển thị lỗi từ Backend (ví dụ: "Email không tồn tại" hoặc "Chức năng chỉ dành cho Admin")
      const msg = err?.response?.data?.error || err?.response?.data?.message || "Không thể gửi email xác thực. Vui lòng thử lại.";
      setError(msg);
    } finally {
      setLoading(false);
    }
        ...
        <div className="relative z-10 w-full h-full flex flex-col justify-center items-center px-12 py-16 backdrop-blur-[1px]">
            ...
            <div className="space-y-4 text-center">
                <h1 className="text-5xl font-black tracking-tighter text-white drop-shadow-md">
                    Khôi Phục <span className="text-[#ffe400]">Tài Khoản</span>
                </h1>
                <p className="max-w-md text-lg font-medium leading-relaxed text-white/80">
                    Đừng lo lắng! Chỉ cần nhập email và chúng tôi sẽ gửi mã OTP để bạn đặt lại mật khẩu ngay lập tức.
                </p>
            </div>
        </div>
        ...
        <div className="w-full max-w-[440px] px-4">
            ...
            <div className="bg-white p-8 sm:p-12 rounded-[2rem] shadow-[0_20px_50px_rgba(156,0,229,0.1)] relative overflow-hidden border border-purple-50">
              ...
              <div className="mb-8 text-center">
                  ...
                  <p className="mt-2 text-gray-500">Nhập email để nhận mã xác thực.</p>
              </div>
              ...
              <div className="pt-6 mt-8 text-center border-t border-gray-100">
                  <button 
                    onClick={() => nav("/login")} 
                    className="text-sm font-semibold text-gray-400 hover:text-[#9c00e5] transition-colors flex items-center justify-center w-full gap-2 group"
                  >
