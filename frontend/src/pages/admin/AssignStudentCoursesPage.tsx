        ...
        <div className="container mx-auto max-w-7xl">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-4">
            <div>
              <div className="flex items-center gap-3 mb-2">
                ...
                <h1 className="text-3xl font-black bg-gradient-to-r from-text-primary to-text-secondary bg-clip-text text-transparent transition-colors duration-300">Phân quyền học sinh</h1>
              </div>
              <p className="text-text-secondary text-sm ml-12">Quản lý và cấp quyền truy cập khóa học cho học sinh.</p>
            </div>
           ...
           <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                <div className="p-6 bg-bg-card border border-color-border rounded-2xl backdrop-blur-sm">
                    <div className="flex justify-between">
                        <div><p className="text-text-secondary text-sm font-bold uppercase">Tổng giao dịch</p><h3 className="text-4xl font-black text-text-primary mt-2">{enrollments.length}</h3></div>
          ...
          <div className="flex flex-col md:flex-row gap-4 mb-6 bg-black/5 dark:bg-white dark:bg-[#1f1428]/50 p-4 rounded-2xl border border-color-border">
                <div className="relative flex-1">
                    ...
                    <input className="w-full pl-12 p-3 bg-bg-card border border-color-border rounded-xl text-text-primary focus:border-[#9c00e5] focus:outline-none" placeholder="Tìm kiếm học sinh theo tên hoặc email..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
                </div>
            ...
            <table className="w-full text-left">
              ...
              <tbody className="divide-y divide-color-border">
                {loading ? <tr><td colSpan={7} className="p-12 text-center text-text-secondary">Đang tải...</td></tr> : 
                 currentEnrollments.length === 0 ? <tr><td colSpan={7} className="p-12 text-center text-text-secondary">Không tìm thấy dữ liệu.</td></tr> :
                 currentEnrollments.map((enr, idx) => {
                    const student = students.find(s => s.id === enr.student_id);
                    const course = courses.find(c => c.id === enr.course_id);
                    return (
                    <tr key={enr.id} className="hover:bg-bg-card transition-all group animate-slide-in" style={{ animationDelay: `${idx * 50}ms` }}>
                      <td className="p-5 pl-6">
                        <div className="flex items-center gap-3">
                            ...
                            <span className="font-bold text-text-primary group-hover:text-[#9c00e5] transition-colors">{student?.full_name || "Unknown"}</span>
                        </div>
                      </td>
                      <td className="p-5 text-text-secondary">{student?.email}</td>
                      <td className="p-5 font-bold text-text-primary">{course?.name || `ID: ${enr.course_id}`}</td>
                      <td className="p-5 text-text-secondary font-mono text-sm">{formatDate(enr.created_at || enr.enrolled_at)}</td>
                      <td className="p-5 text-text-secondary text-sm">
                        <div className="flex flex-col gap-1">
                          {enr.start_at ? <span className="text-green-400">Từ: {formatDate(enr.start_at)}</span> : <span className="text-text-secondary">Từ: ---</span>}
                          {enr.end_at ? <span className="text-red-400">Đến: {formatDate(enr.end_at)}</span> : <span className="text-text-secondary">Đến: ---</span>}
                        </div>
                      </td>
          ...
          {totalPages > 1 && (
             <div className="flex justify-between items-center mt-6 px-4 bg-black/5 dark:bg-white dark:bg-[#1f1428]/30 p-4 rounded-xl border border-color-border">
                <span className="text-sm text-text-secondary">Hiển thị {startIndex + 1}-{Math.min(startIndex + ITEMS_PER_PAGE, filteredEnrollments.length)} / {filteredEnrollments.length}</span>
