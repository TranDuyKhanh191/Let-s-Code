...
export default function AssignCoursesPage() {
  ...
  const handleSaveAssignment = async (data: any) => {
    ...
    try {
      if (data.isEditing) {
        const response = await fetch(`${API_BASE}/api/permissions/${data.id}`, { 
            method: "PATCH", 
            headers, 
            body: JSON.stringify({ 
                status: data.status,
        ...
        if (response.ok) showToast("Cập nhật phân quyền thành công!", "success");
  ...
  const handleHardDelete = async (id: string | number) => {
    if (!confirm("Bạn có chắc chắn muốn xóa VĨNH VIỄN phân quyền này khỏi hệ thống?")) return;
    try {
      const token = localStorage.getItem("token");
      await fetch(`${API_BASE}/api/permissions/${id}`, { method: "DELETE", headers: { Authorization: `Bearer ${token}` } });
                      ...
                      <td className="p-5">
                        <div className="flex justify-center gap-2 opacity-80 group-hover:opacity-100 transition-opacity">
                          {assign.status === 'pending' ? (
                            ...
                            <>
                              <button onClick={() => handleOpenEdit(assign)} className="p-2 bg-yellow-500/10 text-yellow-400 rounded-lg hover:scale-110 transition-all border border-yellow-500/20" title="Chỉnh sửa"><PencilIcon className="w-4 h-4" /></button>
                              <button onClick={() => handleRevoke(assign.id)} className="p-2 bg-red-500/10 text-red-500 rounded-lg hover:scale-110 transition-all border border-red-500/20" title="Thu hồi"><TrashIcon className="w-4 h-4" /></button>
                            </>
                          ) : (
                              <button onClick={() => handleHardDelete(assign.id)} className="p-2 bg-red-500/20 text-red-500 rounded-lg hover:scale-110 transition-all border border-red-500/40" title="Xóa vĩnh viễn khỏi DB"><TrashIcon className="w-4 h-4" /></button>
                          )}
                        </div>
                      </td>
                    </tr>
