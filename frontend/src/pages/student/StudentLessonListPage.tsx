...
const StudentLessonListPage = () => {
  ...
  useEffect(() => {
    const fetchAllData = async () => {
      ...
      try {
        // If course is not provided via location.state, use placeholder
        if (!course) {
           setCourse({ name: "Chi tiết khóa học", code: `COURSE-${id}` });
        }

        // Fetch Lessons for student
        const lessonRes = await fetch(`${API_BASE_URL}/lessons/student/course/${id}`, { headers });
      ...
      <main className="container relative z-10 px-4 pt-8 mx-auto lg:px-8 max-w-7xl">
        {errorMsg ? (
             <div className="flex flex-col items-center justify-center py-32 text-center border border-red-500/20 rounded-[3rem] bg-red-500/5 backdrop-blur-sm">
                ...
                <button onClick={() => navigate('/student/home')} className="px-8 py-3 font-bold text-black bg-[#ffe400] rounded-xl hover:bg-white transition-all">Quay lại trang chủ</button>
            </div>
